...
const Sidebar = ({ className, user }: Props) => {
  ...
  return (
    <div
      className={cn(
        "hidden md:flex flex-col h-full w-[88px] lg:w-[256px] fixed left-0 top-0 px-4 border-r-2 p-[0_16px]",
        className
      )}
    >
      ...
      <div className="flex flex-col gap-y-2 flex-1">
        ...
        <HoverCard openDelay={0}>
          <HoverCardTrigger asChild>
            <Button
              variant={"sidebar"}
              className="p-[4px_8px] text-[calc(var(--type-base-size)-3px)] justify-start h-[52px] gap-0 cursor-default"
            >
              <>
                <Image
                  src={"/more.svg"}
                  alt={"more"}
                  className="lg:ml-[6px] lg:mr-5"
                  height={32}
                  width={32}
                />
          ...
          <HoverCardContent className="w-[252px]" side="right">
            ...
            <div className="p-[8px_0]">
              <button
                type="button"
                className="cursor-pointer flex p-[0_20px] items-center text-[calc(var(--type-base-size)-3px)] min-w-[100px] w-full h-10 justify-normal text-wolf font-bold uppercase hover:bg-background-hover"
              >
                Cài đặt
              </button>
              <button
