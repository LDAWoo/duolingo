...
const Quiz = ({
  ...
  const [selectedMatchOption, setSelectedMatchOption] = React.useState<
    ChallengePartOption[] | []
  >([]);
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      workerRef.current = new Worker(
        new URL(
          "/workers/challenge-progress-workers.js",
          process.env.NEXT_PUBLIC_URL
        )
      );
      userProgressWorkerRef.current = new Worker(
        new URL(
          "/workers/user-progress-workers.js",
          process.env.NEXT_PUBLIC_URL
        )
      );
  ...
  const onContinue = () => {
    switch (challenge.type) {
      ...
      case "FILL":
        ...
        const correctMatchOptions = optionsParts
        ...
        const isCorrectOrder =
          selectedMatchOption.length === correctMatchOptions.length &&
          selectedMatchOption.every(
            (o, index) =>
              o.text === correctMatchOptions[index]?.text &&
              o.correct === correctMatchOptions[index]?.correct
          );
    ...
    <>
      ...
      <div className="flex-1">
        <div className="grid h-full w-full items-center justify-center grid-rows-[minmax(0,1fr)] grid-cols-1 device:grid-cols-[min-content] device:grid-rows-[min-content] p-[24px_16px]">
          <div
            key={challenge.id}
            className="animate-slide-left device:min-h-[450px] device:w-[600px] w-full grid grid-rows-[min-content,minmax(0,1fr)] gap-4 overflow-y-auto overflow-x-hidden device:overflow-y-visible device:gap-6"
          >
            <div className="flex flex-col gap-2">
              <h1 className="font-bold leading-[1.25] text-[calc(var(--type-base-size)+8px)] min-[700px]:text-[calc(var(--type-base-size)+14px)]">
                {getTitle()}
              </h1>
            </div>
            <div className="grid items-start device:items-center mb-2">
              {challenge.type !== "CONVERSATION" &&
                challenge?.challengeQuestions.length > 0 && (
                  <ChallengeQuestion
                    questions={challenge?.challengeQuestions}
              ...
              {challenge.type === "LISTEN" && (
                <QuestionAudio audioSrc={challenge?.audioSrc} />
              )}
              {challenge.type === "CONVERSATION" && (
              ...
              {(challenge.type === "SELECT" ||
                ...
                <Challenge
                  ...
                  disable={!!selectedOption && status !== "none"}
                  type={challenge?.type}
                />
              )}
              {challenge.type === "MATCH" && (
      ...
      <Footer
        loading={loading}
        disable={onDisableButton() || loading}
        status={status}
        answerCorrect={onAnswerCorrect()}
        onCheck={onContinue}
      />
    </>
  );
